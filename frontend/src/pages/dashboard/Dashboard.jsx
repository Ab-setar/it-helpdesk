		...
		<div className='space-y-6'>
			<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
				<div>
					...
					<p className='text-sm text-gray-500 dark:text-gray-400 mt-1'>Overview of all support tickets</p>
				</div>
			</div>

			<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
				<StatCard label='Total Tickets' value={stats.total}
					icon={TicketIcon} iconBg='bg-indigo-100 dark:bg-indigo-900/30'
					iconColor='text-indigo-600 dark:text-indigo-400' valueColor='text-gray-900 dark:text-white' />
				<StatCard label='Open' value={stats.open}
					icon={CircleDot} iconBg='bg-yellow-100 dark:bg-yellow-900/30'
					iconColor='text-yellow-600 dark:text-yellow-400' valueColor='text-yellow-600 dark:text-yellow-400' />
				<StatCard label='In Progress' value={stats.inProgress}
					icon={Clock} iconBg='bg-blue-100 dark:bg-blue-900/30'
					iconColor='text-blue-600 dark:text-blue-400' valueColor='text-blue-600 dark:text-blue-400' />
				<StatCard label='Closed' value={stats.closed}
					icon={CheckCircle2} iconBg='bg-green-100 dark:bg-green-900/30'
					iconColor='text-green-600 dark:text-green-400' valueColor='text-green-600 dark:text-green-400' />
			</div>
						...
						<tbody>
							{tickets.length === 0 ? (
								...
								<tr key={ticket._id} className='border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors'>
									<td className='py-3.5 px-4 font-mono text-xs font-medium text-indigo-600 dark:text-indigo-400'>#{ticket.ticketId}</td>
									<td className='py-3.5 px-4 font-medium text-gray-900 dark:text-white'>{ticket.issueType}</td>
									<td className='py-3.5 px-4'><span className={`badge ${getPriorityBadge(ticket.priority)}`}>{ticket.priority}</span></td>
									<td className='py-3.5 px-4'><span className={`badge ${getStatusBadge(ticket.status)}`}>{ticket.status}</span></td>
									<td className='py-3.5 px-4 text-gray-700 dark:text-gray-300'>{ticket.submitterId?.name || '—'}</td>
									<td className='py-3.5 px-4 text-gray-500 dark:text-gray-400'>{new Date(ticket.createdAt).toLocaleDateString()}</td>
									<td className='py-3.5 px-4'>
										<Link to={`/tickets/${ticket._id}`} className='inline-flex items-center gap-1 text-indigo-600 hover:text-indigo-800 dark:hover:text-indigo-400 text-sm font-medium'>
